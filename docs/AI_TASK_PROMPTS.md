...
```text
你正在参与“AI旅行决策与动态行程助手”项目。请先完整阅读仓库根目录下的AGENTS.md、CONTEXT.md、PROJECT_OVERVIEW.md、TRAVEL_GUIDE_SPEC.md、SCOPE_MATRIX.md、CONTRACTS.md、SHARED_SCHEMA_HANDOFF.md、DATA_PROVIDER_ARCHITECTURE.md、DATA_REQUIREMENTS_CATALOG.md、MODEL_PROVIDER_AND_SECRETS.md、PROJECT_DESIGN.md以及docs/adr中的文件。
...
如果需要的共享Schema尚未由C提供，必须输出SCHEMA_BLOCKER并暂停相关实现，不能自行定义替代对象。
```
...
3. CONTRACTS.md定义的9个MCP工具，先用Mock实现统一返回；
4. Mock/Snapshot/Live/Hybrid Provider接口与切换配置；
...
所有MCP输入输出和ResourceCandidate必须导入共享Schema；缺失时停止并向C询问。
```
...
TripProfile、UserAction、TravelGuide及所有接口响应必须导入共享Schema；缺失时停止并向C询问。
```
...
优先完成：
1. 按SHARED_SCHEMA_HANDOFF.md补齐全部共享Pydantic模型、MCP/REST输入输出、合法/非法fixture和PlanState，并提交schema-v0.4冻结版本；
...
5. 时间窗、总天数、预算和锁定节点验证；
6. 通用重规划子流程、版本谱系，并以至少一种事件完成端到端验收；
7. FastAPI消息、确认、修改和突发情况接口；
8. 使用fake数据完成端到端测试。

不要自由修改A和B的模块。规划算法先追求可解释和稳定，不必追求数学最优。
若CONTRACTS本身存在歧义，停止实现并请求三人拍板；不得自行发明字段。
```

## 集成检查提示词

```text
请作为集成审查者，阅读AGENTS.md、CONTEXT.md、PROJECT_OVERVIEW.md、TRAVEL_GUIDE_SPEC.md、SCOPE_MATRIX.md、CONTRACTS.md、DATA_PROVIDER_ARCHITECTURE.md和PROJECT_DESIGN.md，然后检查当前代码是否满足：
1. 三个模块是否使用同一套Schema；
2. LLM是否可能输出不存在的实体ID或无来源事实；
3. MCP工具是否真的被LangGraph节点调用；
4. RAG结果是否保留evidence_id；
5. 前置过滤、增量检查和最终验证是否都存在；
6. 验证失败是否可能错误设置PlanValidationStatus.VALID；
7. 局部重规划是否保留locked节点；
8. 外部API失败时是否有降级路径。
9. 用户侧最终结果是否是七部分TravelGuide，而不是只返回ItineraryPlan。
10. 计划可行性、数据可信度和攻略生命周期是否分离。
11. 总预算是否只由CostItem[]计算。
12. Mock数据是否进入DataSnapshot并在前端可见。
